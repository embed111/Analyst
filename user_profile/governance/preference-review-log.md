...
## 2026-03-23 协作域超限评审
1. 触发文件：
   1. `user_profile/core/thinking-patterns-domain-collaboration.md`（209 行，超限）。
2. 评审结论：
   1. 本次超限主要来自“分发提示词治理”与“原型说明细则”这类操作级条目，更适合作为短期候选保留。
   2. 长期主档仅保留跨话题稳定、且会直接驱动默认协作策略的核心条目。
3. 降级去向：
   1. `user_profile/core/thinking-patterns-short-term.md`：
      - 分发开工提示词统一起始句式；
      - 提示词专用目录与保留窗口；
...
4. 评审后状态：
   1. `thinking-patterns-domain-collaboration.md` 已回到 200 行内。
5. 备注：
   1. 若上述条目后续继续跨主题稳定影响默认协作策略，可按规则回升到长期主档。

## 2026-03-27 协作域超限评审
1. 触发文件：
   1. `user_profile/core/thinking-patterns-domain-collaboration.md`（206 行，超限）。
2. 评审结论：
   1. 本次新增的“缺陷与 agent 默认中文化”属于 owner 明示且会直接影响默认协作策略，应保留在长期主档。
   2. “聊天类原型优先收成企业 IM 壳”与“默认展示信息做减法”虽已多次出现，但当前更偏展示 / 原型阶段细则，先降到短期偏好池。
3. 降级去向：
   1. `user_profile/core/thinking-patterns-short-term.md`：
      - 聊天壳风格候选；
      - 默认展示减法候选。
4. 评审后状态：
   1. `thinking-patterns-domain-collaboration.md` 已回到 200 行内。
5. 备注：
   1. 若上述两类条目继续跨更多主题稳定影响默认协作策略，可再按规则回升到长期主档。
