...
用途：
1. 保存索引层自身元数据，例如 `schema_version`、`artifact_root_marker`、`last_full_rebuild_at`、`generator_version`。
...
摘要字段：`summary`, `node_count`, `running_node_count`, `success_node_count`, `failed_node_count`, `blocked_node_count`
...
摘要字段：`command_summary`, `latest_event`, `result_summary`, `exit_code`, `event_count`
...
### 6.7 `conversation_message_index`
